...
```
## Scripts 
- ``parameter_def.py`` = Contains the parameter definition for all scripts. There are two type of parameters. 
    -  ``baseline_param()``: Are imported by all scripts. Refer to the definition of the Network structure and the parameters of the dynamics.
    - ``name_of_script_param()``: These parameters are only imported by the script ``name_of_script.py``.

- ``main_simulation.py`` = Runs one simulation of the epidemic-CT app dynamical model using the parameters specified in ``baseline_param()`` and ``main_simulation_param()``. It allows to explore the impact of a parameter of choice (``test_param``) on the effectiveness of the CT app. 
    - ``test_param``: Name of the parameter to explore (*str*). Can be chosen by the user. The most common ones needed are: ``av_reluct_thr``, ``compliance``, ``beta``.
...
- ``main_plot.py``: Uses the output Simulations from ``main_simulation.py`` to plot the temporal evolution of the epidemic and CT app adoption dynamics. It contains two type of plots. 
...
## Cite as: 
Fosch, A., Aleta, A., & Moreno, Y. (2023). Characterising the role of human behaviour in the effectiveness of contact-tracing applications. arXiv preprint arXiv:2307.13157.